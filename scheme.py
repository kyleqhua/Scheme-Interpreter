...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals != nil:
            if vals == nil:
                raise SchemeError('arguments dont match')
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        value = do_lambda_form(Pair(target.second, expressions.second), env)
...
def make_let_frame(bindings, env):
    ...
    get_symbols = lambda binding: binding.first
    get_vals = lambda binding: scheme_eval(binding.second.first,env)
    symbols = bindings.map(get_symbols)
    vals = bindings.map(get_vals)
    while bindings != nil:
        check_form(bindings.first, 2, 2)
        bindings = bindings.second
    check_formals(symbols)
    child = env.make_child_frame(symbols, vals)
    return child
    # END PROBLEM 15
# class MacroProcedure(LambdaProcedure):
#     """A macro: a special form that operates on its unevaluated operands to
...
def do_define_macro(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        value = MacroProcedure(target.second,expressions.second, env)
        env.define(target.first,value)
        return target.first
